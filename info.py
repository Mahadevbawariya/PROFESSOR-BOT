...
BOT_TOKEN = environ['7186384407:AAFPebMgF0OinQ6Dn5PpvxNbAuyVWqOp6BU']
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('1141965474', '').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('-1002018375646', '0').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('-1002061066371')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URL = environ.get('mongodb+srv://cerolec385:GD4neWudB9Nv1IIH@cluster0.akbnu4n.mongodb.net/', "")
